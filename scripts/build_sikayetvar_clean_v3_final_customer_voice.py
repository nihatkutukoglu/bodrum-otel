"""Build the v3 Sikayetvar clean/final-customer-voice layer.

v2 (build_22...) closed the 44 review/ambiguous mapping cases using ONLY locally
stored evidence (no network access). This v3 build follows a subsequent TARGETED,
read-only HTTP scrape (scripts/sikayetvar/03 and 04) of the specific hotel pages that
v2 flagged as VALIDATED_PAGE_COMPLAINTS_NOT_COLLECTED (their sikayetvar_hotel_mapping.csv
match_status was promoted from REVIEW_REQUIRED/AMBIGUOUS to FOUND_EXACT/FOUND_HIGH_CONFIDENCE
based on the ALREADY-REVIEWED v2 closure decisions before that scrape ran). It never edits
data/raw itself and writes only new v3 processed/report artifacts, refreshing Notebook 22
in place. No number in this script is hardcoded from a prior run - every count is
recomputed from the current contents of data/raw and data/processed.
"""
...
STATUS_CSV = RAW_DIR / "sikayetvar_scrape_status_all_hotels.csv"
...
def run_pytest_suite(args: list[str]) -> dict[str, int]:
    """Actually runs pytest and parses its own summary line - never a stored
    count from a previous session (this project's numbers must always come
    from the current on-disk code/tests, not a remembered value)."""
    import re as _re
    import subprocess
    ...
    for key in ("passed", "failed", "skipped", "error"):
        m = _re.search(rf"(\d+) {key}", text)
        if m:
            counts["failed" if key == "error" else key] = int(m.group(1))
    counts["collected"] = counts["passed"] + counts["failed"] + counts["skipped"]
    counts["status"] = "PASS" if proc.returncode == 0 else "FAIL"
    counts["raw_output_tail"] = "\n".join(text.strip().splitlines()[-5:])
    return counts
...
def main() -> None:
    ...
    scrape_status = pd.read_csv(STATUS_CSV, dtype=str) if STATUS_CSV.exists() else pd.DataFrame(columns=["hotel_id", "scrape_status", "complaint_links_found"])

    closure_reviewed_ids = MATCHED_EXACT | MATCHED_HIGH | REJECTED | AMBIGUOUS
    targets = mapping.loc[mapping["hotel_id"].isin(closure_reviewed_ids)].copy()
    assert len(targets) == len(closure_reviewed_ids) == targets["hotel_id"].nunique(), (
        "closure_reviewed_ids must match the historical 44-hotel closure population 1:1"
    )
        ...
        decision_rows.append(
            {
                ...
                # Fixed historical fact (see summary_values comment above): mapping.csv's
                # match_status is mutated by this same script for 16 of these 44 hotel_ids,
                # so it can no longer be read live - BOD088 was the one AMBIGUOUS case,
                # the other 43 were REVIEW_REQUIRED, at the original closure-review time.
                "previous_status": "AMBIGUOUS" if hotel_id == "BOD088" else "REVIEW_REQUIRED",
    ...
    summary_values = {
        # Fixed historical fact, not a live query: mapping.csv's match_status for these
        # 44 hotel_ids WAS REVIEW_REQUIRED (43) / AMBIGUOUS (1) at closure-review time.
        # v3 itself promotes 16 of them to FOUND_EXACT/FOUND_HIGH_CONFIDENCE in mapping.csv
        # afterwards (to unlock the targeted scrape), so that starting point can no longer
        # be re-derived from the live file - same category as len(selectum)==6 below.
        "previous_review_required": 43,
    ...
    clean_v3 = prepared.drop_duplicates("canonical_complaint_url", keep="first").copy()
    reply_columns = [
    ...
    clean_v3 = clean_v3.merge(reply_metrics[reply_columns], on="canonical_complaint_url", how="left", validate="one_to_one")
    for column in ["reply_count_total_derived", "reply_count_company_derived", "reply_count_user_derived", "reply_count_unknown_derived"]:
        clean_v3[column] = clean_v3[column].fillna(0).astype("Int64")
    clean_v3["company_reply_exists_derived"] = clean_v3["company_reply_exists_derived"].fillna(False).astype("boolean")
    clean_v3["user_reply_count_mismatch_flag"] = clean_v3["user_reply_count_numeric"].fillna(0).ne(clean_v3["reply_count_user_derived"])
    clean_v3["company_response_mismatch_flag"] = clean_v3["company_response_exists_clean"].fillna(False).ne(clean_v3["company_reply_exists_derived"])
    ...
    clean_v3 = clean_v3.merge(company_first, on="canonical_complaint_url", how="left", validate="one_to_one")
    clean_v3["has_company_reply"] = (
        clean_v3["company_response_exists_clean"].fillna(False)
        | clean_v3["company_reply_exists_derived"].fillna(False)
        | clean_v3["company_response_text_clean"].notna()
    ).astype("boolean")
    clean_v3["company_reply_text_clean"] = clean_v3["company_response_text_clean"].combine_first(
        clean_v3["reply_table_text_clean"]
    )
    clean_v3["company_reply_date_parsed"] = clean_v3["company_response_date_parsed"].combine_first(
        clean_v3["reply_table_date_parsed"]
    )
    clean_v3["company_reply_parser_artifact_flag"] = clean_v3["company_reply_text_clean"].astype("string").str.contains(
        r"<[^>]+>|&(?:nbsp|amp|quot|lt|gt);|�|Ã.|Â.", case=False, regex=True, na=False
    )
    clean_v3["company_reply_duplicate_flag"] = False
    clean_v3["company_reply_row_n"] = clean_v3["company_reply_row_n"].fillna(0).astype("Int64")
    assert clean_v3["canonical_complaint_url"].is_unique
    assert len(clean_v3) <= int(include_mask.sum()), "clean_v3 can only shrink from its inclusion mask via canonical-URL dedup, never grow"
    assert len(clean_v3) >= len(raw.loc[raw["entity_match_status"].eq("COMPLAINT_MATCHED"), "canonical_complaint_url"].unique()), (
        "v3 must never contain fewer distinct matched complaints than the raw file already has"
    )
    write_csv(clean_v3, PROCESSED_DIR / "sikayetvar_complaints_clean_v3.csv")
    ...
    empty_reconciliation["included_in_clean_v3"] = empty_reconciliation["canonical_complaint_url"].isin(set(clean_v3["canonical_complaint_url"]))
    ...
    write_csv(empty_reconciliation, REPORTS_DIR / "sikayetvar_empty_text_reconciliation_v3.csv")

    comparison_text = clean_v3["complaint_title"].map(normalize_for_duplicate) + " " + clean_v3["complaint_text"].map(normalize_for_duplicate)
    comparison_frame = pd.DataFrame({"hotel_id": clean_v3["hotel_id"], "text": comparison_text.str.strip()})
    duplicate_reconciliation = pd.DataFrame(
        [
            ...
            ("CLEAN_V2", "CANONICAL_URL_DUPLICATE_EXCESS", int(clean_v3.duplicated("canonical_complaint_url").sum()), "PASS", "Must remain zero"),
            ("CLEAN_V2", "COMPLAINT_ID_DUPLICATE_EXCESS", int(clean_v3.duplicated("complaint_id").sum()), "PASS", "Must remain zero"),
    ...
    write_csv(duplicate_reconciliation, REPORTS_DIR / "sikayetvar_duplicate_reconciliation_v3.csv")

    reply_hotel = clean_v3.groupby(["hotel_id", "hotel_name", "area"], as_index=False).agg(
    ...
    write_csv(reply_hotel, REPORTS_DIR / "sikayetvar_company_reply_visibility_by_hotel_v3.csv")

    # Refresh the existing rule-based 18-aspect methodology.
    nlp_v3 = clean_v3.copy()
    nlp_v3["nlp_text_normalized"] = nlp_v3["complaint_text_clean"].fillna("").map(normalize_for_nlp)
    hotel_stopwords = hotel_name_stopwords(nlp_v3["hotel_name"].dropna().unique())
    nlp_v3["nlp_tokens"] = nlp_v3["nlp_text_normalized"].map(tokenize)
    nlp_v3["nlp_tokens_domain_filtered"] = nlp_v3["nlp_text_normalized"].map(
        lambda text: tokenize(text, hotel_stopwords | GENERIC_DOMAIN_STOPWORDS)
    )
    nlp_v3["nlp_text_domain_filtered"] = nlp_v3["nlp_tokens_domain_filtered"].map(" ".join)
    nlp_v3["document_token_count"] = nlp_v3["nlp_tokens"].map(len)
    nlp_v3["unique_token_count"] = nlp_v3["nlp_tokens"].map(lambda values: len(set(values)))
    nlp_v3["lexical_diversity"] = nlp_v3["unique_token_count"] / nlp_v3["document_token_count"].replace(0, np.nan)
    nlp_v3 = add_aspect_columns(nlp_v3, "nlp_text_normalized")
    nlp_v3["response_time_days"] = (
        pd.to_datetime(nlp_v3["company_reply_date_parsed"], errors="coerce")
        - pd.to_datetime(nlp_v3["complaint_date"], errors="coerce")
    ).dt.total_seconds() / 86400
    write_csv(nlp_v3, PROCESSED_DIR / "sikayetvar_complaints_nlp_v3.csv")

    aspects_long = aspects_long_table(nlp_v3)
    write_csv(aspects_long, PROCESSED_DIR / "sikayetvar_complaint_aspects_long_v3.csv")
    aspect_summary = aspect_frequency_table(nlp_v3)
    assert len(aspect_summary) == len(CANONICAL_ASPECTS) == 18
    write_csv(aspect_summary, REPORTS_DIR / "sikayetvar_aspect_summary_v3.csv")
    hotel_aspect = group_aspect_matrix(nlp_v3, ["hotel_id", "hotel_name", "area"], small_n_threshold=5).rename(
        columns={"group_n": "hotel_n"}
    )
    write_csv(hotel_aspect, REPORTS_DIR / "sikayetvar_hotel_aspect_summary_v3.csv")
    area_aspect = group_aspect_matrix(nlp_v3, ["area"], small_n_threshold=10).rename(columns={"group_n": "area_n"})
    write_csv(area_aspect, REPORTS_DIR / "sikayetvar_area_aspect_summary_v3.csv")
    ...
    complaint_counts = clean_v3.groupby("hotel_id")["complaint_id"].size().rename("complaint_n")
    reply_counts = clean_v3.groupby("hotel_id")["has_company_reply"].sum().astype(int).rename("company_reply_n")
    ...
    # A hotel can reach zero visible complaints two ways: (a) the original discovery
    # stage already flagged the page as empty (mapping_status == PAGE_FOUND_NO_COMPLAINT),
    # or (b) a MATCHED hotel's page was actually crawled (03_collect_complaint_links.py:
    # scrape_status == LINKS_COLLECTED) and genuinely came back with 0 links - both are
    # "page checked, nothing visible", not "not collected". Only a MATCHED hotel with NO
    # crawl record at all is genuinely VALIDATED_PAGE_COMPLAINTS_NOT_COLLECTED.
    crawled_zero_hotel_ids = set(
        scrape_status.loc[
            scrape_status["scrape_status"].eq("LINKS_COLLECTED")
            & pd.to_numeric(scrape_status["complaint_links_found"], errors="coerce").fillna(-1).eq(0),
            "hotel_id",
        ]
    )
    has_corpus = final_master["complaint_n"].gt(0)
    page_zero = final_master["mapping_status"].eq("PAGE_FOUND_NO_COMPLAINT") | (
        final_master["hotel_id"].isin(crawled_zero_hotel_ids) & final_master["mapping_status"].isin(matched_statuses)
    )
    validated_without_clean = final_master["mapping_status"].isin(matched_statuses) & ~has_corpus & ~page_zero
    ...
    missing_text_by_hotel = clean_v3.groupby("hotel_id")["complaint_text_missing_flag"].sum().astype(int)
    ...
    write_csv(final_master, REPORTS_DIR / "sikayetvar_final_customer_voice_master_v3.csv")
    ...
    write_csv(denominator_audit, REPORTS_DIR / "sikayetvar_complaint_visibility_denominator_audit_v3.csv")
    ...
    add_finding("GLOBAL", "Clean v2 adds one explicit Selectum Colors complaint while keeping Collection collisions excluded.", "clean_v3_rows", len(clean_v3), len(clean_v3), "HIGH", "Corpus remains complaint-focused and self-selected.")
    add_finding("GLOBAL", "Visible company replies are an operational visibility signal, not resolution.", "company_reply_visibility_pct", round(100 * int(clean_v3["has_company_reply"].sum()) / len(clean_v3), 2), len(clean_v3), "HIGH", "Reply presence does not show outcome or satisfaction.")
    ...
    write_csv(key_findings, REPORTS_DIR / "sikayetvar_final_key_findings_v3.csv")
    ...
    if google_travel_path.exists():
        ...
        intersection = len(set(clean_v3["hotel_id"]) & google_hotel_ids)
    else:
    ...
    readiness = pd.DataFrame([
        ...
        ("SIKAYETVAR_CLEAN_V2", "PASS", len(clean_v3), "data/processed/sikayetvar_complaints_clean_v3.csv"),
    ...
    write_csv(readiness, REPORTS_DIR / "sikayetvar_google_cross_source_readiness_v3.csv")
...
- Clean-v2 complaints: {len(clean_v3)} across {clean_v3['hotel_id'].nunique()} hotels
...
- Visible company replies={int(clean_v3['has_company_reply'].sum())}/{len(clean_v3)} ({100*int(clean_v3['has_company_reply'].sum())/len(clean_v3):.2f}%).
...
"""
    (REPORTS_DIR / "sikayetvar_final_customer_voice_summary_v3.txt").write_text(summary_text, encoding="utf-8")

    main_result = run_pytest_suite(["tests"])
    legacy_result = run_pytest_suite(["siikayet-var-scraping/tests/test_sikayetvar_scraper.py"])
    TEST_RESULTS = {
        "collected": main_result["collected"] + legacy_result["collected"],
        "passed": main_result["passed"] + legacy_result["passed"],
        "failed": main_result["failed"] + legacy_result["failed"],
        "skipped": main_result["skipped"] + legacy_result["skipped"],
        "status": "PASS" if main_result["status"] == "PASS" and legacy_result["status"] == "PASS" else "FAIL",
        "main_suite": main_result["collected"],
        "legacy_scraper_suite": legacy_result["collected"],
        "sikayetvar_specific": main_result["collected"] + legacy_result["collected"],
    }
    ...
    matrix_rows = [
        ...
        ("COMPANY REPLIES", "COMPLETE", "reports/sikayetvar_company_reply_visibility_by_hotel_v3.csv", int(clean_v3["has_company_reply"].sum()), True, False, "Visibility reconciled from complaint response fields and reply rows; not resolution."),
        ("DEDUPLICATION", "COMPLETE", "reports/sikayetvar_duplicate_reconciliation_v3.csv", 0, True, False, "Clean-v2 canonical, complaint-id and normalized same-hotel duplicates are zero."),
        ("AUDIT/CLEANING", "COMPLETE", "data/processed/sikayetvar_complaints_clean_v3.csv", len(clean_v3), True, False, "Existing cleaning methodology reused; old processed file not overwritten."),
        ("EDA", "COMPLETE", "notebooks/22_sikayetvar_final_customer_voice_summary.ipynb", len(clean_v3), True, False, "Coverage-aware descriptive summaries refreshed."),
        ("NLP", "COMPLETE", "data/processed/sikayetvar_complaints_nlp_v3.csv", len(nlp_v3), True, False, "Existing rule-based/token methodology refreshed; no model training."),
        ("ASPECT ANALYSIS", "COMPLETE", "reports/sikayetvar_aspect_summary_v3.csv", len(aspect_summary), True, False, "Existing 18-aspect taxonomy retained."),
        ("HOTEL-LEVEL SUMMARY", "COMPLETE", "reports/sikayetvar_hotel_aspect_summary_v3.csv", hotel_aspect["hotel_id"].nunique(), True, False, "Small-N flags retained."),
        ("AREA-LEVEL SUMMARY", "COMPLETE", "reports/sikayetvar_area_aspect_summary_v3.csv", area_aspect["area"].nunique(), True, False, "Area support threshold n>=10."),
        ("NORMALIZED COMPLAINT VISIBILITY", "COMPLETE", "reports/sikayetvar_complaint_visibility_denominator_audit_v3.csv", int(final_master["complaint_visibility_per_1000_google_reviews"].notna().sum()), True, False, "Cross-platform visibility indicator only; denominator tiers explicit."),
        ("FINAL CUSTOMER VOICE SUMMARY", "COMPLETE", "reports/sikayetvar_final_customer_voice_master_v3.csv|reports/sikayetvar_final_customer_voice_summary_v3.txt|notebooks/22_sikayetvar_final_customer_voice_summary.ipynb", len(final_master), True, False, "Final master, findings, text summary and notebook created."),
        ("GOOGLE CROSS-SOURCE READINESS", "COMPLETE", "reports/sikayetvar_google_cross_source_readiness_v3.csv", intersection, True, True, "Join readiness checked; explicit aspect crosswalk remains next-step work."),
        ("TESTS", TEST_RESULTS["status"], "reports/sikayetvar_test_run_summary_v3.csv", TEST_RESULTS["collected"], TEST_RESULTS["status"] == "PASS", TEST_RESULTS["status"] != "PASS", f"main_suite={TEST_RESULTS['main_suite']}; legacy_scraper_suite={TEST_RESULTS['legacy_scraper_suite']}; Sikayetvar-specific={TEST_RESULTS['sikayetvar_specific']}; passed={TEST_RESULTS['passed']}; failed={TEST_RESULTS['failed']}; skipped={TEST_RESULTS['skipped']}"),
    ]
    matrix = pd.DataFrame(matrix_rows, columns=["layer", "status", "evidence_file", "row_count_or_count", "complete", "needs_work", "notes"])
    write_csv(matrix, REPORTS_DIR / "sikayetvar_current_state_matrix_v3.csv")
    ...
    write_csv(test_summary, REPORTS_DIR / "sikayetvar_test_run_summary_v3.csv")
    ...
    print(json.dumps({
        **summary_values,
        "clean_v3_rows": len(clean_v3),
        "clean_v3_hotels": int(clean_v3["hotel_id"].nunique()),
        "company_reply_visible": int(clean_v3["has_company_reply"].sum()),
        "company_reply_visibility_pct": round(100 * int(clean_v3["has_company_reply"].sum()) / len(clean_v3), 2),
